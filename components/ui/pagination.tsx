...
export const Pagination = ({
  ...
  return (
    <div className="flex items-center justify-between w-full mt-4 select-none">
      <button
        className={clsx(
          "flex items-center gap-1 text-sm border border-[#111b21] bg-[#111b21] text-gray-500 rounded p-2 outline-none",
          {
      ...
      <div className="flex items-center gap-2">
        {pages?.map((page, index) => (
          <span
            key={index}
            className={clsx(
              "flex items-center justify-center h-6 w-6 p-1 text-sm font-normal rounded text-[#111b21] cursor-pointer",
              {
